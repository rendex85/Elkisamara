...
class CategoryManager(models.Manager):
    CATEGORY_NAME_COUNT_NAME = {"Елки": "tree__count", "Смартфоны": "smartphone__count"}
...
class Product(models.Model):
    ...
    price = models.DecimalField(
        max_digits=9, decimal_places=2, verbose_name="Цена, руб"
    )
...
class ChristmasTree(Product):
    height = models.DecimalField(
        max_digits=9,
        decimal_places=2,
        verbose_name="Рост елки, м",
        null=True,
        blank=True,
    )
    tree_type = models.CharField(
        max_length=255, verbose_name="Тип дерева", null=True, blank=True
    )
    weight = models.IntegerField(verbose_name="Вес, кг", null=True, blank=True)
    from_place = models.CharField(
        max_length=512, verbose_name="Откуда привезена", null=True, blank=True
    )
    image = models.ImageField(
        verbose_name="Изображение", upload_to="products/tree", null=True, blank=True
    )
...
class CartProduct(models.Model):
    ...
    final_price = models.DecimalField(
        max_digits=9, decimal_places=2, verbose_name="Общая цена", null=True, blank=True
    )
...
class Cart(models.Model):
    ...
    total_products = models.PositiveIntegerField(
        default=0, verbose_name="Всего товара, шт"
    )
    final_price = models.DecimalField(
        max_digits=9, default=0, decimal_places=2, verbose_name="Общая цена, руб"
    )
    in_order = models.BooleanField(default=False, verbose_name="Оформлен ли заказ?")
    for_anonymous_user = models.BooleanField(
        default=False, verbose_name="Зарегистрован ли пользователь?"
    )
...
class Order(models.Model):
    ...
    phone = models.CharField(
        max_length=20, verbose_name="Телефон", null=True, blank=True, default=None
    )
    cart = models.ForeignKey(Cart, verbose_name="Корзина", on_delete=models.CASCADE)
    address = models.CharField(
        max_length=1024, verbose_name="Адрес", null=True, blank=True
    )
    ...
    order_content_description = models.TextField(
        verbose_name="Описания составляющих заказа", null=True, blank=True
    )

    def __str__(self):
        return (
            f"Заказ номер: {self.id}, дата: {self.created_at.date()},  {self.customer} "
        )
